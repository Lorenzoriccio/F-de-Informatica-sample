...
El teléfono celular cuenta con harware y software al igual que las computadoras. Por ejemplo, la interfaz de los productos Apple no es la misma que aquellos hechos por Android. 
...
_
_prompt ambas: Python 3.10.2 (tags/v3.10.2:a58ebcc, Jan 17 2022, 14:12:15) [MSC v.1929 64 bit (AMD64)] on win32
Type "help", "copyright", "credits" or "license" for more information._ 

El prompt de ambas es el mismo. 

El lenguaje que "entiede" la terminal de Visual Code es python. 
...
El `*` simboliza la cuenta de multiplicacion, por lo tanto el resultado es 15.  
...
El `/` simboliza la cuenta de división, por lo tanto el resultado es 2. 
...
En el último, se refiere a una variable. 
...
El resultado nos devuelve el tipo de dato llamado string ya que se trata de una cadena de caracteres. 
...
La letra "C" tendrá la posición 6 ya que la cuenta se inicia en 0 y el espacio se cuenta como caracter. 
...
>
_saludo = "Hola mundo"_ al invocarlo no devuelve nada ya que se está creando una nueva variable pero... 
...
>
Sí se puede hacer `saludo.upper().lower()` y te devuelve la última función que agregaste, es decir "hola mundo".  
...
Haciendo len(lista) te devuelve la longitud de la lista. 
...
> Para pensar 🤔: Probá la sentencia `list.index("25")` ¿Qué resultado obtenes? ¿Para qué sirve _index()_

La función `list.index` sirve para conocer en qué posición se encuentra un elemento dentro de la lista. `list.index("25")` te devuelve error ya que ese número no pertenece a la lista. 
...
Se consigue invocando el nombre del diccionaario previamente etiquetado. 
...
El ejercicio anterior nos imprime el pedido en la lista_comensales. "i" se puede referir a un contador o a un elemento dentro de la lista. 
